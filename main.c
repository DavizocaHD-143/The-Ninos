...
client_c loc_conf;
internet_c mp_conf[MAX_PLAYERS];


mybool enet = false,enet_configured = false,cmd_show = false;
ENetHost *client = NULL;
ENetPeer *peer = NULL;

float oldx = 0,oldy = 0,oldz = 0,olddirx = 0,olddiry = 0,olddirz = 0;
//hex colors
...
long initapp (long argc, char **argv)
{
	...
	memset(mp_conf, 0, sizeof(mp_conf)); //clear memory
	for (int i = 0;i < MAX_PLAYERS;i++){
		mp_conf[i].connected = false;
	}
...
void doframe ()
{
	...
	//ENET
	if (enet == true && enet_configured == false) {
		if (enet == true && enet_configured == false){
			multiplayer_cmd_init(&loc_conf);
			enet_configured = true;
		}
	}
